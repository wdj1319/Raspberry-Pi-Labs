...
/*
    ...
    std::string garbage;
    std::string correct_input = "ABCDQabcdq";

    control.init_com(3);
    char stars[] = "****************************************\n";
    //print menu screen
    std::cout << ("%s", stars);
    std::cout << ("Tiva C Test Operations, by WYATT JOHNSON\n");
    std::cout << ("%s", stars);
    std::cout << ("(A) Test Analog Input\n");
    std::cout << ("(B) Test Digital Input\n");
    std::cout << ("(C) Test Debounced Button\n");
    std::cout << ("(D) Test Servo Motor\n");
    std::cout << ("(Q)uit\n");

    do
    {
        getline(std::cin, CMD_input);
    } while (CMD_input.size() != 1 || CMD_input.find_first_of(correct_input) == std::string::npos);

    if (CMD_input == "A" || CMD_input == "a")
    {
        double start_time = cv::getTickCount();

        while (!_kbhit())
        {
            hor_percent = control.get_analog(JOYSTICK_X, hor_result);
            ver_percent = control.get_analog(JOYSTICK_Y, ver_result);

            std::cout << "ANALOG TEST: CH" << JOYSTICK_X << "= " << hor_result << " (" << hor_percent << "%) ";
            std::cout << "CH" << JOYSTICK_Y << "= " << ver_result << " (" << ver_percent << "%)\n";
            cv::waitKey(1);
        }
        _getch();

    }
    if (CMD_input == "B" || CMD_input == "b")
    {
        int button_press;
        double start_time = cv::getTickCount();
        ...
        while (!_kbhit())
        {
            control.get_data(DIGITAL, BUTTON_1, button_press);
            if (button_press == 0)
            {
                control.set_data(DIGITAL, BLUE_RGB, 1);
            }
            else
            {
                control.set_data(DIGITAL, BLUE_RGB, 0);
            }
            std::cout << "DIGITAL TEST: CH" << BLUE_RGB << "= " << button_press << "\n";
            cv::waitKey(1);
        }
        _getch();
        control.set_data(0, BLUE_RGB, 0);
    }
    if (CMD_input == "C" || CMD_input == "c")
    {
        int result;
        int num_presses = 0;
        double start_time = cv::getTickCount();
        ...
        while (!_kbhit())
        {
            if (control.get_button(BUTTON_1, result))
            {
                num_presses++;
                std::cout << "BUTTON TEST : " << num_presses << "\n";
            }
            cv::waitKey(1);
        }
        _getch();
...
void lab4()
{
    cv::Size canvas_size(1000, 500);
    CSketch sketch(canvas_size, 3);
    sketch.run();
...
{
    cv::Size canvas_size(1000, 500);
    CSpaceInvaderGame spaceinvader(canvas_size, 3);
    spaceinvader.run();
}
...
void lab7()
{
    cv::namedWindow("Exit Screen", cv::WINDOW_AUTOSIZE);
    ...
    if (CMD_input == "B" || CMD_input == "b")
    {
        ...
        while (cv::waitKey(1) != 'q')
        {
            ...
            std::cout << "DIGITAL TEST: CH" << BLUE_LED << "= " << button_press << "\n";
            cv::waitKey(1);
        }
